...
const getLocalSuffix = (countryName: string) => {
  ...
  return ['Journal', 'Review', 'Insights']; // For Canada, US, UK, AU, etc.
};
...
const buildSEOPrompt = (niche: string, country: string, serpLinks: string[], trendData: number[]) => {
  ...
  **STRICT INSTRUCTIONS**:
  ...
  2. If real local websites are missing, invent 8 realistic local sounding websites for ${countryName} (e.g., "${niche} Journal" or "${niche} Hub").
...
export async function generateSEOReport(niche: string, country: string) {
  ...
  checklist.slice(0, 15).forEach((item, i) => markdown += `${i+1}. ${item}\n`);
  markdown += `\n10. GROWTH ACCELERATORS\n──────────────────────────────────────────────────────────────\n`;
  accelerators.slice(0, 5).forEach((tip, i) => markdown += `${i+1}. ${tip}\n`);
  markdown += `\n11. RELATED RESOURCES\n──────────────────────────────────────────────────────────────\n`;
  resources.slice(0, 8).forEach((res, i) => markdown += `${i+1}. ${res}\n`);
