...
const buildSEOPrompt = (niche: string, country: string, serpContext: string, trendData: number[]) => {
  ...
  **REAL DATA INPUT FROM ALL APIs**:
  ${serpContext}
  
  **STRICT INSTRUCTIONS**:
  1. Use these REAL competitor URLs and Titles to identify actual local brands. NEVER output 'Review 1', 'Journal', 'Dergisi', or '$72'. CPC must be between $0.50 and $10.00.
  2. If real local websites are missing from the context, DO NOT invent fake sites. Say: 'SERP data currently unavailable. Focus on actionable strategies.'
  3. Strict Country Lock: Do not mention US, UK, or other countries. Only ${countryName}.
  4. Provide a clear 30/60/90 Actionable Plan.
  
  **Return ONLY a valid JSON object. No markdown blocks, no extra text.**
  Return JSON: key_insights (3), immediate_actions (3), trend_summary, trend_assessment, keywords (50), serp_landscape (8 OR honest disclaimer), content_roadmap (12), link_acquisition (target_sites + guest_post_topics), onpage_checklist (15), growth_accelerators (5), related_resources, local_market_context (3), local_business_base (4), actionable_plan (3), client_value_proposition (3), swot_analysis (4), action_priority_matrix (5), risk_assessment (3), financial_projection (2), final_ceo_summary (3), data_limitations (3).`;
};
...
export async function generateSEOReport(niche: string, country: string) {
  ...
  // 🔥 ALL APIS: ScraperAPI First, SerpAPI Second, Serper Third (Full Data Extraction)
  ...
  if (searchData?.organic_results) {
    const topSites = searchData.organic_results.slice(0, 8).map((r: any) => 
      `Title: ${r.title} | URL: ${r.link} | Snippet: ${r.snippet || ''}`
    ).join('\n');
    serpContext = `Here are the top real competitors found via Google SERP:\n${topSites}`;
  }

  const prompt = buildSEOPrompt(niche, country, serpContext, trendData);
  ...
  // 🆕 NAYE SECTIONS: SWOT, Risk, ROI, CEO Summary
  const swot = ensureStringArray(analysis.swot_analysis);
  markdown += `14. SWOT ANALYSIS\n──────────────────────────────────────────────────────────────\n`;
  swot.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;

  const matrix = ensureStringArray(analysis.action_priority_matrix);
  markdown += `15. ACTION PRIORITY MATRIX (IMPACT vs. EFFORT)\n──────────────────────────────────────────────────────────────\n`;
  matrix.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;

  const risk = ensureStringArray(analysis.risk_assessment);
  markdown += `16. RISK ASSESSMENT (HIGH/MEDIUM/LOW)\n──────────────────────────────────────────────────────────────\n`;
  risk.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;

  const financial = ensureStringArray(analysis.financial_projection);
  markdown += `17. FINANCIAL PROJECTION (MODELED ESTIMATE)\n──────────────────────────────────────────────────────────────\n`;
  financial.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;

  const ceo = ensureStringArray(analysis.final_ceo_summary);
  markdown += `18. FINAL CEO SUMMARY\n──────────────────────────────────────────────────────────────\n`;
  ceo.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;

  const limits = ensureStringArray(analysis.data_limitations);
  markdown += `19. DATA LIMITATIONS & ASSUMPTIONS\n──────────────────────────────────────────────────────────────\n`;
  limits.forEach((item: string, i: number) => markdown += `  ${i+1}. ${item}\n`);
  markdown += `\n`;
