...
export const createTechnicalSEOReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // If blocked, generate incomplete audit report
    if (isBlocked) {
      const today = new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' });
      const reference = `MKT-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;

      const markdown = `MusePRO\nReal-Time Market Research\nIntelligence Division\n════════════════════════════════════════════════════════════════════\nTECHNICAL SEO AUDIT REPORT\n════════════════════════════════════════════════════════════════════\n\n**Prepared For:** [Client Name]\n**Date:** ${today}\n**Audit Timestamp (UTC):** ${auditTimestamp}\n**Prepared By:** MusePRO SEO Team\n**Reference:** ${reference}\n**Classification:** CONFIDENTIAL\n════════════════════════════════════════════════════════════════════\n\nAUDIT INCOMPLETE – SITE BOT PROTECTION DETECTED\n────────────────────────────────────────────────────────────────────\n\nWe attempted to crawl ${websiteUrl} on ${auditTimestamp}, but the server returned HTTP ${status}.\n\n**What this means:**\nYour website is protected by a bot management system (e.g., Cloudflare, Incapsula) or a firewall that blocks automated audit tools. This prevents us from accessing the HTML, CSS, and other resources needed to perform a complete technical SEO analysis.\n\n**Why this matters for SEO:**\nGooglebot and other search engine crawlers may also be blocked or served limited content, which can severely impact your organic rankings. If search engines cannot access your pages, they cannot index or rank them.\n\n**Recommended Next Steps:**\n1. Whitelist our audit bot's IP address or user agent in your firewall/CDN settings.\n2. Temporarily disable bot protection during the audit window, or allow our specific user agent.\n3. Verify that Googlebot is not being blocked by testing your site with Google Search Console's "Fetch as Google" tool.\n4. Contact us at support@musepro.com with the whitelisting details, and we will re-run the audit immediately.\n\n**What we could not check due to the block:**\n- On-page elements (title, meta, H1, images alt)\n- Technical foundation (robots.txt, sitemap, canonical, structured data)\n- Performance metrics (page speed, Core Web Vitals)\n- Security headers and mixed content\n- Internal linking structure\n\n**Data Credibility:**\nThis report contains no scores, estimates, or recommendations because the necessary data could not be retrieved. We will provide a complete analysis once access is granted.\n\n════════════════════════════════════════════════════════════════════\nThis report is generated by MusePRO Senior Research Division.\n════════════════════════════════════════════════════════════════════`;
      ...
      const report = await Report.create({
        ...
        data: {
          ...
          isBlocked: true,
          auditTimestamp,
          note: 'Audit incomplete due to bot protection. No technical checks performed.',
        },
        markdown,
        charts: {},
        traffic_estimate: 0,
    ...
    // If page fetched successfully, perform full audit
    ...
    const headers = (html as any)?.headers || {};
    securityHeaders = {
    ...
    // Check robots.txt and sitemap.xml (only if not already blocked)
    try {
    ...
    // Build checks with business impact
    const checks: AuditCheck[] = [
      ...
      },
      {
        ...
        recommendation: hasJSONLD ? 'Ensure schema type matches page content.' : 'Add appropriate JSON-LD schema (Organization, Product, Article, etc.) for rich snippets.'
      },
      {
        name: 'Canonical Tag',
        passed: hasCanonical,
    ...
    // Revenue impact (only if site accessible and we have some basis)
    ...
    const estimatedRevenueLossMonthly = Math.round(estimatedMonthlyTrafficLoss * 0.02 * 100); // assuming $100 AOV and 2% conversion
