...
export const createTechnicalSEOReport = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const checks: AuditCheck[] = [
      ...
      { name: 'X-Robots-Tag Header', passed: securityHeaders['X-Robots-Tag'] !== '' && !/noindex/i.test(securityHeaders['X-Robots-Tag']), measured: true, impactScore: 9, effort: 'Low', evidence: securityHeaders['X-Robots-Tag'] || 'Missing', recommendation: /noindex/i.test(securityHeaders['X-Robots-Tag']) ? 'Remove noindex directive from X-Robots-Tag to allow indexing.' : securityHeaders['X-Robots-Tag'] ? 'Header is set and does not block indexing.' : 'Add X-Robots-Tag if you need to control indexing.' },
